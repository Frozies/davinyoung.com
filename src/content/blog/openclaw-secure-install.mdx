...
- An existing **Traefik reverse proxy** on your network with Cloudflare DNS challenge for TLS (your `securetap-docker` setup).
...
- Replace `claw.sunsetvista.co` with your actual domain.
...
ssh-keygen -t ed25519 -a 100 -C "davin@laptop-$(date +%Y%m%d)" -f ~/.ssh/id_ed25519
```
...
> **Note:** Port 18789 is only open to the Traefik host's LAN IP — not to the internet. Public access goes through Traefik (`claw.sunsetvista.co` on port 443), and private access goes through the Tailscale mesh. Tailscale traffic is always encrypted with WireGuard and authenticated via your Tailnet identity.
...
- **Traefik** at `https://claw.sunsetvista.co` (Phase 6) — for any device
...
```
https://claw.sunsetvista.co/#token=<your-token>
```
...
#### 5.3.1 Remove Stray Root-Level Auth Keys

The wizard may have placed `mode` and `token` keys inside the root `auth` block. These belong under `gateway.auth`, not at the root. Remove them:
...
The gateway auth is correctly at `gateway.auth.mode` and `gateway.auth.token` — leave those as-is.
...
> **Note:** Traefik is the primary way to access the OpenClaw dashboard in this setup. Because the gateway uses `bind=lan` (required for cross-host Traefik), Tailscale Serve is not available. If all your access is from Tailscale devices, you can still reach the dashboard directly at `http://10.0.0.91:18789` from within your LAN/tailnet, but Traefik provides TLS and security headers.

This guide assumes you already run a **Traefik reverse proxy on a separate Docker host** with Cloudflare DNS challenge for TLS certificates — the same setup that fronts your other services. The OpenClaw VM (`10.0.0.91`) is a backend target, not a Traefik host. Nothing is installed on the OpenClaw VM in this phase.

### 6.1 Architecture

```
Internet → Cloudflare → Traefik host (Docker) → 10.0.0.91:18789 (OpenClaw VM)
                         ├── claw.sunsetvista.co  → OpenClaw gateway
                         ├── panel.games.sunsetvista.co → Pterodactyl
                         ├── api.securetap.io → Spring Boot API
                         └── ... other services
```
...
In your Cloudflare dashboard for `sunsetvista.co`:
...
```bash
dig claw.sunsetvista.co +short
```

### 6.3 Traefik Configuration

These changes go on your **Traefik Docker host** (not the OpenClaw VM). The files are in your `securetap-docker/external/` directory.
...
curl -I https://claw.sunsetvista.co/

# From the Traefik host — test direct LAN access
curl -s -o /dev/null -w "%{http_code}" http://10.0.0.91:18789/

# From the OpenClaw VM — test localhost still works
curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:18789/
```

You should see HTTPS responses with all security headers through `claw.sunsetvista.co`, and HTTP 200/401 responses on the direct LAN and localhost checks (depending on whether the token is provided).
...
```
https://claw.sunsetvista.co/#token=<YOUR_GATEWAY_TOKEN>
```
...
MailFrom = logwatch@claw.sunsetvista.co
Detail = Med
Range = yesterday
Service = All
LWEOF
...
| TLS certificate check | Monthly | `curl -vI https://claw.sunsetvista.co 2>&1 \| grep expire` |
...
- [ ] `claw.sunsetvista.co` DNS record created in Cloudflare
- [ ] Traefik router `claw-secure` added to `config.yml`
- [ ] Traefik service `claw-service` points to `http://10.0.0.91:18789`
...
- [ ] `curl -I https://claw.sunsetvista.co/` returns security headers
