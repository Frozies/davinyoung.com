...
- A Tailscale account ([login.tailscale.com](https://login.tailscale.com)), which is free for personal use with up to 100 devices.
...
3. **Installation type:** **Ubuntu Server (minimized)**, which is the smallest footprint
4. **Network:** Configure your interface. Use a **static IP** if possible, since it makes everything easier:
...
> **Note:** Yes, we are using password auth for this first connection. Phase 1 will disable password auth and switch to key-only. This is the bootstrap, and you need one password login to deploy your keys.
...
> **Note:** Do NOT install `open-vm-tools-desktop`. This is a headless server, and there is no GUI.
...
The `-a 100` flag increases the KDF rounds for the key passphrase, making brute-force harder. **Always set a passphrase** on your key, because an unprotected private key on a stolen laptop gives the attacker direct server access.
...
```bash
# Disable Avahi/mDNS daemon if present (OpenClaw has its own mDNS, which we disable too)
...
| Fail2Ban is your only brute-force defense | No brute-force possible, since Tailscale authenticates at the network layer before TCP connects |
...
3. Check **Reusable**, which allows the same key to be used after reboots.
...
> **Note:** With an auth key, there is no browser URL step. The node joins the tailnet immediately. If you prefer interactive authentication instead (browser-based), omit `--authkey` and run `sudo tailscale up --ssh`, and you will be given a URL to open in your browser each time the node needs to re-authenticate.
...
You should see your server listed with a `100.x.y.z` Tailscale IP address. Note this IP, because you will use it throughout the rest of this guide.
...
> **Note:** This is a strict upgrade over traditional SSH key auth. Your Tailscale identity is tied to your SSO provider (Google, GitHub, Okta), which supports MFA. Traditional SSH keys are bearer tokens, so anyone who copies the private key file gets access. Tailscale SSH eliminates this risk.
...
Expected output should show `"lan"` for bind and `"token"` for auth mode. Both are required, because LAN binding without token auth is a critical vulnerability.
...
Deep audit (probes the live gateway, so start it temporarily):
...
> **Warning:** Because Traefik runs on a different host, the OpenClaw gateway **cannot** bind to `loopback`. It must bind to `lan` so the Traefik host can reach it over the `10.0.0.x` network. Token authentication is **mandatory** when binding to LAN, and this is already configured in Phase 5. This also means **Tailscale Serve cannot be used**, because OpenClaw enforces `bind=loopback` when Tailscale Serve/Funnel is active, which is incompatible with cross-host proxying.
...
> **Note:** Do NOT use `sudo ufw allow 18789/tcp`, because that opens the port to everyone. The `from` clause restricts it to the Traefik host only.
...
> **Note:** `allowTailscale` is not set by the wizard, which means it defaults to `false`, and that is correct for this setup. Only add it explicitly if the security audit flags it.
...
When you first open the dashboard through Traefik, the gateway enforces device pairing. This is the `dmPolicy: "pairing"` security setting, which means each new browser/device must be approved before it can access the control UI.
...
> **Note:** Each new browser or device you use to access the dashboard will require this pairing process. This is by design, and it prevents unauthorized browsers from accessing the control UI even if they have the token. The pairing is stored persistently and survives gateway restarts.
...
```json
{
  ...
  "gateway": {
    ...
    // should never expose. Uses exact command-name matching only, so
...
- [ ] `gateway.bind` is set to `"lan"` (required because Traefik is on a separate host)
